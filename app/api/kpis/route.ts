...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      national: { total: r.total, gmv: r.gmv, complete: r.complete,
        cancel: r.cancel, processing: r.processing, txActive: r.tx_active, maxDate, minDate },
      regions: REGION_ORDER.map(name => {
        const row = reg.rows.find(r => r.region === name);
        if (!row) return null;
        return { region: name, total: row.total, gmv: row.gmv,
