...
export async function GET() {
  try {
    ...
    try {
      ...
      const meta = await client.query("SELECT value FROM metadata WHERE key='last_driver_import'");
      return NextResponse.json({ total: r.rows[0].total, lastImport: meta.rows[0]?.value ?? null });
    } finally { client.release(); }
  } catch (err: unknown) {
