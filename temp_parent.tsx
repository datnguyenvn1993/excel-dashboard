...
// GöÇGöÇGöÇ Types GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
interface KPIResult {
...
const ALL_REGIONS = ["Hß+ô Ch+¡ Minh", "H+á Nß+Öi", "Miß+ün Nam", "Miß+ün Bß¦»c"] as const;
const REGION_ORDER = ["Hß+ô Ch+¡ Minh", "H+á Nß+Öi", "Miß+ün Nam", "Miß+ün Bß¦»c"];
const REGION_EMOJIS: Record<string, string> = {
  "Hß+ô Ch+¡ Minh": "=ƒÅÖn+Å", "H+á Nß+Öi": "=ƒÅ¢n+Å", "Miß+ün Nam": "=ƒî¦", "Miß+ün Bß¦»c": "G¢¦n+Å",
};
...
// GöÇGöÇGöÇ Helpers GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
function formatGMV(v: number) {
  if (v >= 1e9) return (v / 1e9).toFixed(2) + " Tß+ë";
...
function getRelativeStr(ts: string | null, now: number): string {
  ...
  if (diff < 1) return "(vß+½a xong)";
  if (diff < 60) return "(" + diff + " ph+¦t tr¦¦ß+¢c)";
  const h = Math.floor(diff / 60), m = diff % 60;
  return "(" + h + " giß+¥" + (m > 0 ? " " + m + " ph+¦t" : "") + " tr¦¦ß+¢c)";
}
...
function ScreenshotBtn({ targetRef, isDark, watermarkText }: { targetRef: React.RefObject<HTMLDivElement | null>; isDark: boolean; watermarkText?: string }) {
  ...
  return (
    <button data-html2canvas-ignore="true" onClick={async () => {
      ...
      {st === "busy" ? "GÅ¦" : st === "done" ? "G£à" : "=ƒô+"} {st === "busy" ? "Xß+¡ l++..." : st === "done" ? "-É+ú copy!" : "Chß+Ñp h+¼nh"}
    </button>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className={`rounded-xl w-full max-w-sm shadow-2xl border ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
        <div className={`px-6 pt-5 pb-2 border-b ${isDark ? "border-gray-700" : "border-gray-100"}`}>
          <h3 className={`font-semibold text-sm ${isDark ? "text-gray-100" : "text-gray-800"}`}>GÜán+Å {title}</h3>
        </div>
        <div className="px-6 py-4"><p className={`text-sm ${isDark ? "text-gray-300" : "text-gray-600"}`}>{message}</p></div>
        <div className="px-6 pb-5 flex justify-end gap-3">
          <button onClick={onCancel} className={`px-4 py-2 text-sm rounded-lg border ${isDark ? "border-gray-600 text-gray-300 hover:bg-gray-700" : "border-gray-300 text-gray-700 hover:bg-gray-50"}`}>Hß+ºy</button>
          <button onClick={onConfirm} className="px-4 py-2 text-sm rounded-lg bg-red-600 text-white hover:bg-red-700 font-medium">X+íc nhß¦¡n x+¦a</button>
        </div>
      </div>
    </div>
  );
...
function DeltaBadge({ delta }: { delta: number | undefined }) {
  ...
  return (
    <div className={`text-[10px] mt-0.5 font-semibold ${up ? "text-green-500" : "text-red-500"}`}>
      {up ? "Gû¦" : "Gû+"} {Math.abs(delta).toFixed(1)}% vs D-7
    </div>
  );
}
...
function KPIRow({ kpi, isDark }: { kpi: KPIResult; isDark: boolean }) {
  ...
  const cards = [
    ...
    { label: "Tß+òng -æ¦ín", value: fmt(t), color: "gray", delta: deltaPct(t, kpi.d7Total) },
    { label: "% Ho+án th+ánh", value: pctC.toFixed(1) + "%", color: "green" },
    { label: "AOV", value: fmt(aov), color: "purple" },
    { label: "-É¦ín hß+ºy", value: fmt(kpi.cancel), color: "red" },
    { label: "% Hß+ºy", value: pctX.toFixed(1) + "%", color: "orange" },
...
// GöÇGöÇGöÇ Main Component GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
export default function Dashboard({ onImportNew, refreshKey = 0, currentUser }: DashboardProps) {
  ...
  }, []); // stable GÇö no selectedHour dep
  ...
  }, [refreshKey, fetchAll, fetchTable]); // fetchTable is now stable GÇö won't re-fire on hour change
  ...
  const handleDriverFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const iSap = headers.indexOf("M+ú SAP");
      const iTT = headers.indexOf("Trß¦íng th+íi");
      const iTTTK = headers.indexOf("Trß¦íng th+íi t+ái khoß¦ún");
      const iDepot = headers.indexOf("Depot");
      const iDoi = headers.indexOf("-Éß+Öi");
      if ([iSap, iTT, iTTTK, iDepot, iDoi].some(i => i === -1)) {
        alert("Kh+¦ng t+¼m thß¦Ñy -æß+º cß+Öt: M+ú SAP, Trß¦íng th+íi, Trß¦íng th+íi t+ái khoß¦ún, Depot, -Éß+Öi");
        return;
      }
      ...
      if (json.error) { alert("Lß+ùi: " + json.error); return; }
      await fetchDriverInfo(); await fetchTeamReport();
    } catch (err) {
      alert("Lß+ùi -æß+ìc file: " + String(err));
  ...
  const todayShort = chartData?.todayDate?.slice(5).replace("-", "/") ?? "GÇö";
  const d7Short = chartData?.d7Date?.slice(5).replace("-", "/") ?? "GÇö";
  ...
  return (
    <div className={`min-h-screen ${bg}`}>
      {confirm?.type === "reset" && (
        <ConfirmDialog title="X+¦a to+án bß+Ö data" isDark={isDark}
          message="Bß¦ín c+¦ chß¦»c muß+æn x+¦a to+án bß+Ö dß+» liß+çu? H+ánh -æß+Öng n+áy kh+¦ng thß+â ho+án t+íc."
          onConfirm={handleResetConfirmed} onCancel={() => setConfirm(null)} />
      )}
      {confirm?.type === "deleteRows" && (
        <ConfirmDialog title="X+¦a c+íc d+¦ng -æ+ú chß+ìn" isDark={isDark}
          message={`Bß¦ín c+¦ chß¦»c muß+æn x+¦a ${confirm.count.toLocaleString()} d+¦ng -æ+ú chß+ìn?`}
          onConfirm={handleDeleteRowsConfirmed} onCancel={() => setConfirm(null)} />
      )}

      {/* GöÇGöÇ Header GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ */}
      <div className={`sticky top-0 z-20 border-b px-6 py-3 shadow-sm`} style={{ background: isDark ? "#1a8a8b" : "#27BDBE", borderColor: isDark ? "#178384" : "#22a7a8" }}>
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="min-w-0">
            <h1 className={`text-sm font-bold uppercase tracking-wide ${textPri}`}>=ƒôè B+üO C+üO Vß¦¼N H+ÇNH PLATFORM</h1>
            <p className={`text-xs mt-0.5 ${textSec}`}>
              {loading ? "-Éang tß¦úi..." : isEmpty ? "Ch¦¦a c+¦ dß+» liß+çu" : `${(natKPI?.total ?? 0).toLocaleString()} -æ¦ín -+ ${todayLabel}${selectedHour !== null ? ` -+ ${String(selectedHour).padStart(2, "0")}:00` : ""}`}
            </p>
            {importTimeStr && (
              <p className={`text-xs ${importIsStale ? "text-red-500" : (isDark ? "text-gray-500" : "text-gray-400")}`}>
                Cß¦¡p nhß¦¡t l+¦c: {importTimeStr} {importRelStr}
              </p>
            )}
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select value={selectedHour ?? ""} onChange={e => handleHourChange(e.target.value === "" ? null : parseInt(e.target.value, 10))}
              className={`text-xs px-2 py-1.5 rounded-lg border ${isDark ? "bg-gray-700 border-gray-600 text-gray-200" : "bg-white border-gray-300 text-gray-700"}`}>
              <option value="">Cß¦ú ng+áy</option>
            ...
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => driverFileRef.current?.click()} disabled={importingDrivers}
                ...
                {importingDrivers ? "-Éang import..." : "Cß¦¡p nhß¦¡t ds tx"}
              </button>
      ...
      {loading ? (
        ...
        <div className={`flex flex-col items-center justify-center h-64 gap-4 ${textSec}`}>
          <p>Ch¦¦a c+¦ dß+» liß+çu. Vui l+¦ng upload file Excel.</p>
        ...
        <div className="p-6 space-y-6 w-[90%] mx-auto">
          ...
          {/* GöÇGöÇ National KPI GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ */}
          <div ref={nationalRef} className={`rounded-xl border p-5 shadow-sm ${cardCls}`}>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <span className="text-lg">=ƒîÉ</span>
                <h2 className={`font-bold text-sm uppercase tracking-wide ${textPri}`}>To+án Quß+æc</h2>
                <span className={`text-xs ${textSec}`}>({(natKPI?.total ?? 0).toLocaleString()} -æ¦ín)</span>
              </div>
          ...
          {/* GöÇGöÇ Regions GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ */}
          {(kpiData?.regions.length ?? 0) > 0 && (
            <div ref={regionsRef} className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className={`font-bold text-sm uppercase tracking-wide ${textSec}`}>Theo Khu Vß+¦c</h2>
                ...
                {kpiData!.regions.map(r => (
                  <div key={r.region} className={`rounded-xl border p-4 shadow-sm ${cardCls}`}>
                    <div className="flex items-center gap-2 mb-3">
                      <span>{REGION_EMOJIS[r.region] ?? "=ƒôì"}</span>
                      <h3 className={`font-bold text-xs uppercase tracking-wide ${textPri}`}>{r.region}</h3>
                      <span className={`text-xs ${textSec}`}>({r.total.toLocaleString()} -æ¦ín)</span>
                    </div>
          ...
          {/* GöÇGöÇ GMV Hourly chart GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ */}
          {chartData && (
            <div ref={hourlyRef} className={`rounded-xl border p-5 shadow-sm ${cardCls}`}>
              <div className="flex items-center justify-between mb-1">
                <h3 className={`font-semibold text-sm ${textPri}`}>
                  =ƒôê GMV theo giß+¥ GÇö <span className="text-blue-500">H+¦m nay ({todayShort})</span> vs <span className={textSec}>D-7 ({d7Short})</span>
                </h3>
                <ScreenshotBtn targetRef={hourlyRef} isDark={isDark} watermarkText={watermarkText} />
              </div>
              <p className={`text-xs mb-4 ${textSec}`}>-É¦ín vß+ï: Triß+çu VN-É</p>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={chartData.hourly} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  ...
                  <Line type="monotone" dataKey="today" name={`H+¦m nay (${todayShort})`} stroke="#3b82f6" strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 5 }}>
          ...
          {/* GöÇGöÇ Team Report GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ */}
          {teamReport.length > 0 && (
            <div ref={teamReportRef} className={"rounded-xl overflow-hidden border " + (isDark ? "border-gray-700" : "border-gray-200")}>
              <div className={"flex items-center justify-between px-4 py-3 " + (isDark ? "bg-gray-800" : "bg-gray-50")}>
                <h3 className={"text-sm font-semibold " + (isDark ? "text-white" : "text-gray-800")}>B+ío c+ío theo -æß+Öi</h3>
                ...
                <table className={"w-full text-xs " + (isDark ? "bg-gray-900 text-gray-200" : "bg-white text-gray-700")}>
                  <thead>
                    <tr className={isDark ? "bg-gray-800 text-gray-400" : "bg-gray-50 text-gray-500"}>
                      <th className="px-3 py-2 text-left">-Éß+Öi</th>
          ...
          {/* GöÇGöÇ Driver Active by Hour GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ */}
          {txHourly.length > 0 && (
            <div ref={driverChartRef} className={`rounded-xl border overflow-hidden ${cardCls}`}>
              <div className={`flex items-center justify-between px-4 py-3 ${isDark ? "bg-gray-800" : "bg-gray-50"}`}>
                <h3 className={`text-sm font-semibold ${textPri}`}>=ƒÜù Driver Active theo giß+¥</h3>
              ...
              <div className={`p-4 ${isDark ? "bg-gray-900" : "bg-white"}`}>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={driverChartData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                    ...
                    {/* FIX: no fragment wrapper GÇö Recharts needs Line as direct child, not inside React fragment */}
                    {!showTeamLines && (
                ...
                {showTeamLines && (
                  <p className={`text-xs mt-1 text-center ${textSec}`}>Click t+¬n tr+¬n legend -æß+â bß¦¡t/tß¦»t -æ¦¦ß+¥ng</p>
                )}
              </div>
            </div>
          )}

          {/* GöÇGöÇ Daily chart GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ */}
          {chartData && (
            <div ref={dailyRef} className={`rounded-xl border p-5 shadow-sm ${cardCls}`}>
              <div className="flex items-center justify-between mb-4">
                <h3 className={`font-semibold text-sm ${textPri}`}>=ƒôà Sß+æ -æ¦ín theo ng+áy (10 ng+áy gß¦ºn nhß¦Ñt)</h3>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={chartData.daily} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  ...
                  <Tooltip {...tt} />
                  <Legend wrapperStyle={{ fontSize: 12 }} />
                  <Line type="monotone" dataKey="complete" name="Ho+án th+ánh" stroke="#10b981" strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 5 }}>
                  ...
                  <Line type="monotone" dataKey="cancel" name="Hß+ºy" stroke="#ef4444" strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 5 }}>
          ...
          {/* GöÇGöÇ Table GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ */}
          <div className={`rounded-xl border shadow-sm overflow-hidden ${cardCls}`}>
            <div className={`px-4 py-2 border-b flex items-center justify-between gap-3 flex-wrap ${isDark ? "bg-gray-900 border-gray-700" : "bg-gray-50 border-gray-100"}`}>
              <span className={`text-xs ${textSec}`}>
                {tableLoading ? "-Éang tß¦úi..." : `${(tableData?.total ?? 0).toLocaleString()} d+¦ng -+ trang ${tablePage + 1}/${totalPages || 1}`}
              </span>
              <div className="flex items-center gap-2">
                {selectedIds.size > 0 && (
                  <button onClick={() => setConfirm({ type: "deleteRows", count: selectedIds.size })}
                    ...
                    =ƒùæ X+¦a {selectedIds.size.toLocaleString()} d+¦ng
                  </button>
                )}
                <button onClick={() => setTablePage(p => Math.max(0, p - 1))} disabled={tablePage === 0 || tableLoading}
                  className={`text-xs px-2.5 py-1.5 rounded border disabled:opacity-40 ${isDark ? "border-gray-600 text-gray-300" : "border-gray-300 text-gray-600"}`}>GåÉ Tr¦¦ß+¢c</button>
                <button onClick={() => setTablePage(p => Math.min(totalPages - 1, p + 1))} disabled={tablePage >= totalPages - 1 || tableLoading}
                  className={`text-xs px-2.5 py-1.5 rounded border disabled:opacity-40 ${isDark ? "border-gray-600 text-gray-300" : "border-gray-300 text-gray-600"}`}>Sau GåÆ</button>
              </div>
            </div>
