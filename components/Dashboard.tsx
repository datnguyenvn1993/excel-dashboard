...
type KPIResult = {
  gmv: number; total: number; complete: number; cancel: number;
  processing: number; pctComplete: number; pctCancel: number; aov: number;
};
...
async function captureToClipboard(el: HTMLElement, isDark: boolean): Promise<void> {
  ...
  const canvas = await h2c(el, { scale: 2, useCORS: true, backgroundColor: isDark ? "#111827" : "#f9fafb" });
...
function KPIRow({ kpi, isDark }: { kpi: KPIResult; isDark: boolean }) {
  ...
  return (
    <div className="grid grid-cols-7 gap-2">
      {cards.map((c) => <KPICard key={c.label} label={c.label} value={c.value} color={c.color} isDark={isDark} />)}
    </div>
  );
}
    ...
    <div className={`min-h-screen ${bg}`}>
      <div className={`sticky top-0 z-20 border-b ${headerBg} px-6 py-3 shadow-sm`}>
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2 shrink-0">
            <button onClick={() => setIsDark((v) => !v)} className={`px-3 py-1.5 text-sm rounded-lg border transition-colors ${isDark ? "border-gray-600 text-yellow-400 hover:bg-gray-700" : "border-gray-300 text-gray-600 hover:bg-gray-100"}`}>
              {isDark ? "☀️ Light" : "🌙 Dark"}
            </button>
            <button onClick={onImportNew} className={`px-3 py-1.5 text-sm rounded-lg border transition-colors ${isDark ? "border-gray-600 text-gray-300 hover:bg-gray-700" : "border-gray-300 text-gray-700 hover:bg-gray-50"}`}>📤 Import mới</button>
            <button onClick={onClearData} className={`px-3 py-1.5 text-sm rounded-lg border transition-colors ${isDark ? "border-red-800 text-red-400 hover:bg-red-900/30" : "border-red-200 text-red-600 hover:bg-red-50"}`}>🗑 Xóa data</button>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className={`rounded-xl border shadow-sm overflow-hidden ${cardCls}`}>
          <div className={`px-4 py-2 border-b text-xs ${isDark ? "bg-gray-900 border-gray-700 text-gray-500" : "bg-gray-50 border-gray-100 text-gray-500"}`}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredRows.slice(0, 500).map((row, i) => {
                  ...
                  const rowBg = i % 2 === 0 ? (isDark ? "bg-gray-800" : "bg-white") : (isDark ? "bg-gray-800/60" : "bg-gray-50/40");
                  return (
