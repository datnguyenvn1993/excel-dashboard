          ...
          {(kpiData?.depots.length ?? 0) > 0 && (
            <div ref={depotsRef} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                {kpiData!.depots.map(d => (
                  <div key={d.depot} className={`rounded-xl border p-4 shadow-sm ${cardCls}`}>
                    <div className="flex items-center gap-2 mb-3">
                      <span>🏢</span>
                      <h3 className={`font-bold text-xs uppercase tracking-wide ${textPri}`}>{d.depot}</h3>
                      <span className={`text-xs ${textSec}`}>({d.total.toLocaleString()} đơn)</span>
                    </div>
                    <KPIRow kpi={d} isDark={isDark} />
                  </div>
                ))}
              </div>
            </div>
